...
// Inject annotator token — falls back to main session token for multi-role accounts
annotatorClient.interceptors.request.use(config => {
  const token = localStorage.getItem('ml_annotator_token') || localStorage.getItem('ml_token')
  if (token) {
...
annotatorClient.interceptors.response.use(
  ...
  async err => {
    ...
    if (err.response?.status === 401 && !err.config._retried) {
      const refresh = localStorage.getItem('ml_annotator_refresh') || localStorage.getItem('ml_refresh')
      if (refresh) {
        try {
          err.config._retried = true
          const res = await axios.post('/api/v1/auth/refresh', { refresh_token: refresh })
          const newToken = res.data.access_token
          // Update whichever token storage is in use
          if (localStorage.getItem('ml_annotator_token')) {
